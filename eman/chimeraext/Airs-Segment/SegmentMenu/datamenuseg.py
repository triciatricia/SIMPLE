...
#       2006.03.03: Lavu Sridhar, BCM (VU 1.2179 Chimera)
#
# To Do:
...
class Segment_Open_Data:
    ...
    def update_data_items(self):
        ...
        for data_region in data_regions:
            #VU 1.2129
            data = find_data_from_data_region(data_region)
...
class Segment_Open_Output:
    ...
    def remove_region_volume_viewer(self, data_item):
        ...
        dr = find_region_volume_viewer(data_item)
        if dr:
            data = find_data_from_data_region(dr)
            msg = 'Segment Volume dialog region -'
            msg = msg + '%s %s' % (data.name, dr.name)
    ...
    def data_show_cb(self):
        ...
        if data_item.region == None:
            from VolumeViewer import volume_from_grid_data
            data_item.region = volume_from_grid_data(data_item.data)
...
def open_models_list():
    ...
    VolumeViewer.surface.Surface_Model OR
    <type '_surface.Surface_Model'> (newer versions)
    """

    # @ For Chimera release version greater than 1.2176,
    #   the data class changed to _surface.Surface_Model
    #   from VolumeViewer.surface.Surface_Model

    from VolumeViewer import Volume
    from _volume import Volume_Model    # solid rendering model
    open_models = [m for m in chimera.openModels.list()
                   if isinstance(m, Volume) or isinstance(m, Volume_Model)]
...
def find_region_volume_viewer(data_item):
    ...
    for data_region in data_regions:
        #VU 1.2129
        data = find_data_from_data_region(data_region)
        if (data.path == data_item.path and
...
def find_model_data_region(data_region, open_models):
    ...
    # for newer versions
    name = '%s %s' %(data.name, data_region.name)
    # for older versions
    name2 = '%s%s' %(data.name, data_region.name)
...
# -------------------------------------------------------------------------
# @ Note that Chimera release version greater than 1.2129 have
#   significant changes in volume data set class.
# -------------------------------------------------------------------------

def find_data_from_data_region(data_region):
    """find_data_from_data_region(data_region)
    ...
    Return grid data for data_region.
    """

    if (data_region == None): return None

    # @ For Chimera release version greater than 1.2129,
    #   grid data is data_region.data where as for older
    #   versions, grid data is data_region.data_set.data.
...
def get_data_region_xform(data_region):
    """get_data_region_xform(data_region)
    ...
    Output:
        xform           Chimera Xform
        tf              data's ijk to xyz_transform
                        (added for Chimera VU 1.2179)

    Return xform and tf for data_region,
    where tf is data's ijk to xyz transform.
    """

    if (data_region == None): return None, None

    # @ For Chimera release version greater than 1.2129,
    #   xform is data_region.model_transform() where as
    #   for older versions, it is data_region.transform().
    ...
    try:
        ...
        # @ We use the fact that tf == None in SegmentSimple,
        #   so, ensure code doesn't break when making changes
        tf = None
...
def get_data_region_bounds(data_region):
    ...
    Output:
        bounds          volume xyz_bounds (old: xyz_region)
        
    Return volume bounds for volume data.
    """

    if (data_region == None): return None

    # @ For Chimera release version greater than 1.2129,
    #   bounds is data_region.xyz_region() where as
    #   for older versions, it is data_region.xyz_bounds().

    #VU 1.2129
    try:
        bounds = data_region.xyz_bounds()
    except:
