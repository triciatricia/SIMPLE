...
class Volume_Path_Dialog(ModelessDialog):
  ...
  def data_region(self):
    ...
    if d == None or not hasattr(d, 'focus_region'):
      return None

    return d.focus_region
...
class Slice:
  ...
  def reset_thresholds(self):
    ...
    dclist = map(lambda cp: cp.data_component(), self.data_region.components)
    for dc in dclist:
      if hasattr(dc, 'volume_path_threshold'):
	delattr(dc, 'volume_path_threshold')
...
class Visible_Data:
  ...
  def __init__(self, data_region, component_display_parameters):

    self.data_region = data_region
    self.component_display_parameters = component_display_parameters
    self.data_component = component_display_parameters.data_component()

  # ---------------------------------------------------------------------------
  #
  def interpolated_value(self, xyz):

    matrix = self.data_region.matrix(self.data_component, None,
				     read_matrix = 0)
  ...
  def threshold(self):
    ...
    if hasattr(self.data_component, 'volume_path_threshold'):
      return self.data_component.volume_path_threshold

    cparams = self.component_display_parameters

    if self.data_region.surface and cparams.surface_levels:
      threshold = min(cparams.surface_levels)
    elif self.data_region.solid and cparams.solid_levels:
      threshold = min(cparams.solid_levels)[0]
    else:
      threshold = None
  ...
  def set_threshold(self, threshold):
    ...
    self.data_component.volume_path_threshold = threshold

  # ---------------------------------------------------------------------------
  #
  def using_displayed_threshold(self):

    return not hasattr(self.data_component, 'volume_path_threshold')
  ...
  def volume_rgba(self, data_value = None):
    ...
    if data_region.surface and cparams.surface_levels:
      tclist = map(lambda t,c: (t,c),
		   cparams.surface_levels, cparams.surface_colors)
      tclist.sort()
      if data_value != None:
	...
	rgba = tclist[0][1]
    elif data_region.solid and cparams.solid_levels:
      tclist = map(lambda tf,c: (tf[0],c),
		   cparams.solid_levels, cparams.solid_colors)
      tclist.sort()
      if data_value != None:
...
class Trace:
  ...
  def __init__(self, visible_data, xyz_in, xyz_out,
    ...
    dc = visible_data.data_component
    self.trace = slice_data_values(dr, dc, xyz_in, xyz_out)
...
def visible_data_components(data_region):
  ...
  cplist = filter(lambda cp: not cp.hidden, data_region.components)
  return map(lambda cparams, dr=data_region: Visible_Data(dr, cparams), cplist)

# -----------------------------------------------------------------------------
#
def slice_data_values(data_region, data_component, xyz_in, xyz_out):

  matrix = data_region.matrix(data_component, None, read_matrix = 0)
  if type(matrix) == type(None):
    return []
