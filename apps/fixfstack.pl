...
3) box size
4) output stack body name (giving <stackbody>.fim )
5) local execution or cluster ( l or c )\n
...
into the <stackbody>.fim SIMPLE stack format. The <stackbody>.hed 
...
if ( $exec =~ 'l' ) {
    ...
    stack_fouriers( "tempfiltab.dat", $nptcls, $ftsize, $body );
} elsif ( $exec =~ 'c' ) {
